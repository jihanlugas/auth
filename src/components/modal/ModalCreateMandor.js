...
const loadingSelect = isFetchingSelector([formActions.MANDOR_DATA.type]);

const mapStateToProps = (state) => {
    return {
        login: state.login,
        currData: state.formikData.get(formActions.MANDOR_DATA.type, {}),
        isFetching: loadingSelect(state.isFetching),
        submitResult: state.submitResult.getIn([formActions.MANDOR_DATA.type, "success"]),
    }
}

const ModalCreateUser = ({ show, onClickOverlay, dispatch, selectedId, currData, isFetching, submitResult, login }) => {
    ...
    const handleDelete = (id) => {
        dispatch(formActions.delete(formActions.MANDOR_DATA, { id }));
    }
    ...
    useEffect(() => {
        if (show) {
            ...
            dispatch(formActions.request(formActions.MANDOR_DATA, {
                id: selectedId
            }, callbackActions));

            return () => {
                dispatch(formActions.clearData(formActions.MANDOR_DATA));
            }
        }
    }, [show]);
            ...
            <LoadingOnFetch isFetching={isFetching} data={currData}>
                <Formik
                    ...
                    onSubmit={(values, actions) => {
                        ...
                        if (selectedId === 0) {
                            dispatch(formActions.create(formActions.MANDOR_DATA, values));
                        } else {
                            dispatch(formActions.update(formActions.MANDOR_DATA, values));
                        }
                    }}
                >
